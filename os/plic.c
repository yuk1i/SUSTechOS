...
void plicinithart(void) {
    ...
    *(volatile uint32 *)(PLIC_SENABLE(ctx) + off * 4) |= (1 << bit);

    if (!on_vf2_board) {
        // set enable bits for this hart's S-mode for the virtio disk.
        off = VIRTIO0_IRQ / 32;
        bit = VIRTIO0_IRQ % 32;
        *(volatile uint32 *)(PLIC_SENABLE(ctx) + off * 4) |= (1 << bit);
    }
    ...
    *(volatile uint32 *)PLIC_SPRIORITY(ctx) = 0;
...
int plic_claim(void) {
    ...
    int irq = *(volatile uint32 *)PLIC_SCLAIM(ctx);
    return irq;
}
...
void plic_complete(int irq) {
    ...
    *(volatile uint32 *)PLIC_SCLAIM(ctx) = irq;
}
